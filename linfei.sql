...
create table if not exists `linfei_column`(
	...
	`mid` char(20) not null COMMENT '模型id',
	-- `default` char(20) not null default 'index.html' COMMENT '默认页名称',
	...
	`status` tinyint(1) not null default 0 COMMENT '栏目状态 0在前端显示 1不显示',
...
--文章表
  create table if not exists `linfei_article`(
   ...
   `column` int unsigned not null COMMENT '所属栏目',
