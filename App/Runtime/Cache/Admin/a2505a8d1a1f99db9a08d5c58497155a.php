                    ...
                    <div class="col-xs-12">
                        ...
                        <form class="form-horizontal"  action="" method="post" enctype="multipart/form-data">
                             ...
                             <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-right" for="form-field-1">组名称<span style="color:#f00;">*</span></label>
                                <div class="col-sm-9"><input class="col-sm-12" type="text" name="name" value="<?php echo ($group); ?>"></div>
                            </div>
                            <div class="form-group">
                                 <label class="col-sm-2 control-label no-padding-right" for="form-field-1"></label>
                              <div class="col-sm-9" >
                                <h5 style="display:inline-block;"><b>全选<b></h5>
                                &nbsp;<input type="checkbox" style="width:14px;height:14px;" id="qxs">
                            </div>
                            <?php if(is_array($adminmenu)): foreach($adminmenu as $k=>$v): ?><div class="form-group" >
                                <label class="col-sm-2 control-label no-padding-right" for="form-field-1"></label>
                              <div class="col-sm-9">
                                <h5 style="display:inline-block;"><b><?php echo ($v['name']); ?><b></h5>
                                &nbsp;<input type="checkbox" style="width:14px;height:14px;" class="xuanzhe" f="ch<?php echo ($k); ?>" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <?php if(is_array($v['child'])): foreach($v['child'] as $key=>$vv): ?><h5 style="display:inline-block;"><?php echo ($vv['name']); ?></h5>&nbsp;<input type="checkbox" name="permissions[]" value="<?php echo ($vv['c']); ?>@<?php echo ($vv['f']); ?>" class="ch<?php echo ($k); ?>" <?php if(in_array($vv['c'].'@'.$vv['f'],$grouplist)){echo 'checked';} ?> style="width:14px;height:14px;">&nbsp;&nbsp;&nbsp;&nbsp;<?php endforeach; endif; ?>
                                </div>
...
<script type="text/javascript">
  $('.xuanzhe').on('click', function() {
        if($(this).prop('checked')) {
            $('.'+$(this).attr('f')+':checkbox').prop('checked', true);
        } else {
            $('.'+$(this).attr('f')+':checkbox').prop('checked', false);
        }
    });
