...
<head>
    <meta charset="utf-8">
	 <title><?php echo ($articlelist['title']); ?></title>
    <meta name="keywords" content="<?php echo ($articlelist['keywords']); ?>">
   <meta name="description" content="<?php echo ($articlelist['description']); ?>" /> 
...
<!--顶area end-->
<div class="topbg">
                <?php $Model = new \Think\Model(); $sql = "select * from linfei_adv where pid = 112"; $adv = $Model->query($sql); foreach($adv as $key=>$v):?><img src="<?php echo ($v['img']); ?>"><?php endforeach;?>
</div>
...
<div class="navigate"><a href="index.php">首页</a> &gt; <span>成功案例</span></div>
        ...
        <!--nfocus-->
<?php if(is_array($article)): foreach($article as $k=>$v): if(!$k){ ?>
        <div class="nfocus mt25">
                <img src="<?php echo homeimg($v['id'],674,322,'article');?>" class="limg" alt="<?php echo ($v['title']); ?>">
                <div class="m">
                    <h2 class="t mt35"><a title="<?php echo ($v['title']); ?>"  href="<?php echo ($v['url']); ?>"><?php echo getsubstr($v['title'],0,9);?></a></h2>
                    <p class="ldes mt30"><?php echo getsubstr($v['description'],0,150);?></p><a href="<?php echo ($v['url']); ?>" class="seall">更多>></a></div></div>
        <!--nfocus end-->
...
<?php }else{ ?>                                  <!--item-->
                    <div class="item clearfix">
                        <img src="<?php echo homeimg($v['id'],328,202,'article');?>" alt="<?php echo ($v['title']); ?>">
                        <div class="m">
                            <h4 class="t"><a href="<?php echo ($v['url']); ?>"><?php echo getsubstr($v['title'],0,22);?></a></h4>
                            <p class="ldes mt10"><?php echo getsubstr($v['description'],0,200);?></p><a href="<?php echo ($v['url']); ?>" target="_blank" class="seall">更多>></a></div>
                    </div>
